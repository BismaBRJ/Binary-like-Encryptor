...
window.title("Binary-like Encryptor (version 0.1.0)")

# Input Label

input_label = tkinter.Label(window, text="Input")
...
def encrypt_button_function():
    ...
    output_text.insert(tkinter.END, encryption)

encrypt_button = tkinter.Button(window, text="Encrypt",
...
def decrypt_button_function():
    ...
    output_text.delete("1.0", tkinter.END)
    output_text.insert(tkinter.END, decryption)

decrypt_button = tkinter.Button(window, text="Decrypt",
...
input_text = tkinter.Text(window, width=60, height=7)
input_text.grid_configure(row=1, column=0, rowspan=1, columnspan=3)

input_text.insert(tkinter.END, """Welcome!
...
input_text_scrollbar = tkinter.Scrollbar(window)
